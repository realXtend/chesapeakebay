...
var FishAvatarController = Class.extend({
    init: function() {
        ...
        this.direction_ = new float3(0,0,0);
        this.dashing_   = false;
        this.canDash_   = true;
        this.speed_     = 0;
        ...
        */
        ...
        else if(me.name == 'FishAvatar_'+client.GetConnectionID()) {
            this.myAvatar_ = true;
    ...
    remove: function() {
        if(this.myAvatar_) {
            ...
            var fishGameEntity = scene.GetEntityByName('FishGame');
    ...
    //     }
    //     var avatarCamera = scene.GetEntityByName('AvatarCamera');
//         var camera = avatarCamera.GetComponent('EC_OgreCamera');
    //     if (camera) {
    //         camera.SetActive();
    //     }
    // },
    ...
    serverInit: function() {
        ...
        if (waterEntity) {
            this.waterLevel_ = waterEntity.waterplane.position.y;
        }
        else
    ...
    serverUpdate: function(dt){
        ...
        if(this.direction_.y != 0) {
            var rot = new float3(0,0,0);
            rot.y = TURNING_SPEED*this.direction_.y*dt;
        ...
        var velocity = new float3(this.speed_,0,0);
        
        var delta = velocity.Mul(dt);
        delta = placeable.LocalToWorld().MulDir(delta);
        
        var newPos = transform.pos.Add(delta);
        var posOnMap = this.terrain_.GetPointOnMap(newPos);
            
        // Stay in water    
        if(newPos.y > this.waterLevel_ - WATER_DELTA)
            newPos.y = this.waterLevel_ - WATER_DELTA;
        else if(newPos.y < posOnMap.y + TERRAIN_DELTA)
            newPos.y = posOnMap.y + TERRAIN_DELTA;
            
        if(this.waterLevel_ > posOnMap.y + WATER_DELTA)
        {
    ...
    serverHandleMouseX: function(param) {
        ...
        rotateVec.y = -MOUSE_SENSITIVITY_X * move;
    ...
    playerClientInit: function() {        
        ...
        // Create fishcamera
        this.fishCamera_ = scene.CreateLocalEntity(['EC_Camera', 'EC_Placeable']);
        this.fishCamera_.SetName("FishCamera");
        this.fishCamera_.SetTemporary(true);
        
        var camera    = this.fishCamera_.camera;
...
//        camera.AutoSetPlaceable();
        ...
        this.mouth_ = scene.CreateEntity(scene.NextFreeIdLocal(), ['EC_RigidBody', 'EC_VolumeTrigger', 'EC_Placeable']);
        ...
        //radar hud
        engine.IncludeFile("fish_game_radar.js"); //creates the radar which updates itself
    },
    ...
    playerClientUpdateFishCamera: function() {
        ...
        var offsetVec = new float3(-0.2, 0, 0);
        offsetVec = avatarplaceable.LocalToWorld().MulDir(offsetVec);
        cameratransform.pos = avatartransform.pos.Add(offsetVec);
        // Note: this is not nice how we have to fudge the camera rotation to get it to show the right things
        //cameratransform.rot.x = -avatartransform.rot.x;
        cameratransform.rot.y = avatartransform.rot.y - 90;
    
        cameraplaceable.transform = cameratransform;

        //var a = new float3(0, 0, 0);
        //var b = new float3(0, 100, 0);
        var a = cameratransform.pos;
        var b = avatartransform.pos;
        //cameratransform.pos, avatartransform.pos
        scene.ogre.DebugDrawLine(a, b, 1, 0, 0, false);
        scene.ogre.DebugDrawLine(avatartransform.pos, new float3(0, 100, 0), 0, 1, 0, false);
    },
